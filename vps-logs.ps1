...
$servicePart = if ([string]::IsNullOrWhiteSpace($Service)) { "" } else { " $Service" }
$cmd = "cd /opt/salarysafe; docker compose logs $followFlag --tail=$Tail$servicePart"

& plink.exe -ssh "root@$ip" -pw $pw -batch -hostkey $KnownHostKey $cmd
if ($LASTEXITCODE -ne 0) { throw "Remote logs command failed" }