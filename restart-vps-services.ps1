...
$cmd = if ($Rebuild) {
  "cd /opt/salarysafe; docker compose up -d --build; docker compose ps"
} else {
  "cd /opt/salarysafe; docker compose restart; docker compose ps"
}

& plink.exe -ssh "root@$ip" -pw $pw -batch -hostkey $KnownHostKey $cmd
if ($LASTEXITCODE -ne 0) { throw "Remote restart failed" }

Write-Host "Services updated on $ip"